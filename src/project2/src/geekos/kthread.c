	...
	int priority, bool detached)
{
    ...
    //Print("DEBUG: Setting refcount to %i\n", kthread->refCount);
...
{
    //Print("DEBUG: Entering Detach_Thread\n");
    //Print("DEBUG: refcount=%i\n",kthread->refCount);
    KASSERT(!Interrupts_Enabled());
    KASSERT(kthread->refCount > 0);

    //Print("DEBUG Setting refcount from %i to %i\n",kthread->refCount,kthread->refCount-1);
    --kthread->refCount;
    if (kthread->refCount == 0) {
	Reap_Thread(kthread);
    }
    //Print("DEBUG Leaving Detach_Thread\n");
}
